...
class TreeModelTask final : public AbstractTreeModel {
    Q_OBJECT

public:
    TreeModelTask(const Info* info, QSharedPointer<Sql> sql, const SectionRule* section_rule, const TableHash* table_hash, const Interface* interface,
...
public slots:
    ...
    void RUpdateOneTotal(int node_id, double, double, double quantity_debit_diff, double quantity_credit_diff) override;
...
private:
    ...
    void UpdateBranchTotal(const Node* node, double quantity_diff);
